...
#pragma region Windows Form Designer generated code
			 ...
			 void InitializeComponent(void)
			 {
				 ...
				 this->tableLayoutPanel1->Size = System::Drawing::Size(1008, 700);
				 ...
				 this->flowLayoutPanel1->Location = System::Drawing::Point(3, 38);
				 this->flowLayoutPanel1->Name = L"flowLayoutPanel1";
				 this->flowLayoutPanel1->Size = System::Drawing::Size(1002, 659);
				 ...
				 this->pictureBox_SourceImage->Size = System::Drawing::Size(484, 640);
				 ...
				 this->pictureBox_OutputImage->Size = System::Drawing::Size(500, 643);
				 ...
				 this->ClientSize = System::Drawing::Size(1008, 700);
...
#pragma endregion
	...
	private: System::Void fastFourierTransformToolStripMenuItem_Click(System::Object^  sender, System::EventArgs^  e) {		//FFT
		int w = dataManager->GetImageWidth();
		int h = dataManager->GetImageHeight();
		// 利用傅立葉之平移性，平移頻率
		for (int i = 0; i < h; i++)
	...
	private: System::Void inverseFastFourierTransformToolStripMenuItem_Click(System::Object^  sender, System::EventArgs^  e) {	//IFFT
		int w = dataManager->GetImageWidth();
		int h = dataManager->GetImageHeight();
	...
	private: System::Void highpassFilterToolStripMenuItem_Click(System::Object^  sender, System::EventArgs^  e) {
		...
		//fourierTransformMethod->FastFourierTransform(dataManager->GetInputImage(), dataManager->GetOutputImage(), dataManager->GetFreqReal(), dataManager->GetFreqImag(), h, w);
		fourierTransformMethod->HighpassFilter(dataManager->GetFreqReal(), dataManager->GetFreqImag(), dataManager->GetOutputImage(), h * 25 / 8, h, w);

		//fourierTransformMethod->InverseFastFourierTransform(dataManager->GetInputImage(), dataManager->GetOutputImage(), dataManager->GetFreqReal(), dataManager->GetFreqImag(), h, w);

		//將算出頻率資訊傳入輸出影像
		Bitmap^ highpass = gcnew Bitmap(w, h);
		for (int i = 0; i < h; i++)
		{
			for (int j = 0; j < w; j++)
			{
				...
				highpass->SetPixel(j, i, Color::FromArgb(valuePixeli, valuePixeli, valuePixeli));
			}
		}
		pictureBox_OutputImage->Image = highpass;
	}
	private: System::Void lowpassFilterToolStripMenuItem_Click(System::Object^  sender, System::EventArgs^  e) {
		...
		//fourierTransformMethod->FastFourierTransform(dataManager->GetInputImage(), dataManager->GetOutputImage(), dataManager->GetFreqReal(), dataManager->GetFreqImag(), h, w);
		fourierTransformMethod->LowpassFilter(dataManager->GetFreqReal(), dataManager->GetFreqImag(), dataManager->GetOutputImage(), h * 25 / 8, h, w);
		//fourierTransformMethod->InverseFastFourierTransform(dataManager->GetInputImage(), dataManager->GetOutputImage(), dataManager->GetFreqReal(), dataManager->GetFreqImag(), h, w);

		//將算出頻率資訊傳入輸出影像
		Bitmap^ lowpass = gcnew Bitmap(w, h);
		for (int i = 0; i < h; i++)
		{
			for (int j = 0; j < w; j++)
			{
				...
				lowpass->SetPixel(j, i, Color::FromArgb(valuePixeli, valuePixeli, valuePixeli));
			}
		}
		pictureBox_OutputImage->Image = lowpass;
	}
